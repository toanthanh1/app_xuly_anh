...
*   **`data/GeminiApiClient.java`**: Lớp giao tiếp trực tiếp với Google Generative AI SDK. Sử dụng model `gemini-2.5-flash` (CẤM THAY ĐỔI).
...
### Cấm Nâng cấp Model
Trong `GeminiApiClient.java`, tên model **BẮT BUỘC** phải là `gemini-2.5-flash`. Theo quy định ngày 01/05/2026, không được phép thay đổi sang các phiên bản khác.
